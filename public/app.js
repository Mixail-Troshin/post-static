...
$("#loginForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    $("#loginErr").textContent = e2.name === "AbortError"
...
$("#addBtn").onclick = async () => {
  ...
  try {
    const r = await api("/api/articles", { method: "POST", body: JSON.stringify({ url }) });
    $("#urlInput").value = "";
    await loadData();
    $("#addMsg").textContent = r.duplicate ? "Уже в списке" : "Готово";
  } catch (e) {
    $("#addMsg").textContent = "Ошибка: " + e.message;
  } finally {
    setTimeout(() => $("#addMsg").textContent = "", 1600);
  }
};
